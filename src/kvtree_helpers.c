 ...
 *
 * Returns KVTREE_SUCCESS if conversion is successful,
 * and KVTREE_FAILURE otherwise.
 *
...
int kvtree_abtoull(char* str, unsigned long long* val)
{
  /* check that we have a string */
  if (str == NULL) {
    kvtree_err("kvtree_abtoull: Can't convert NULL string to bytes @ %s:%d",
            __FILE__, __LINE__
    );
    return KVTREE_FAILURE;
  }
  ...
  /* check that we have a value to write to */
  if (val == NULL) {
    kvtree_err("kvtree_abtoull: NULL address to store value @ %s:%d",
            __FILE__, __LINE__
    );
    return KVTREE_FAILURE;
  }
  ...
  if (errno != 0) {
    /* conversion failed */
    kvtree_err("kvtree_abtoull: Invalid double: %s errno=%d %s @ %s:%d",
            str, errno, strerror(errno), __FILE__, __LINE__
    );
    return KVTREE_FAILURE;
  }
  if (str == next) {
    ...
    kvtree_err("kvtree_abtoull: Invalid double: %s @ %s:%d",
  ...
  if (*next != '\0') {
    ...
    case 'E':
      ...
      break;
    default:
      kvtree_err("kvtree_abtoull: Unexpected byte string %s @ %s:%d",
              str, __FILE__, __LINE__
      );
      return KVTREE_FAILURE;
    }
  ...
  if (num < 0) {
    kvtree_err("kvtree_abtoull: Byte string must be positive: %s @ %s:%d",
            str, __FILE__, __LINE__
    );
    return KVTREE_FAILURE;
  }
  ...
  if (val_d > max_d) {
    /* overflow */
    kvtree_err("kvtree_abtoull: Byte string overflowed UNSIGNED LONG LONG type: %s @ %s:%d",
            str, __FILE__, __LINE__
    );
    return KVTREE_FAILURE;
  }
...
void* kvtree_align_malloc(size_t size, size_t align)
{
  ...
  /* allocate memory */
  void* start = KVTREE_MALLOC(bytes);
  if (start == NULL) {
    return NULL;
  }
